...
function renderProjectBacklog(stories, members) {
            ...
            <div class="sprint-section" data-drop-target-for-element="true">
                <div class="sprint-header">
                    <div class="sprint-left">
                        <label class="sprint-checkbox-label">
                            <input type="checkbox" class="sprint-checkbox" aria-label="Seleccionar todas las actividades en sprint Tablero Sprint 1">
                        ...
                        <button class="sprint-toggle" aria-expanded="true" onclick="toggleSprintSection(this)" title="Contraer">
                        ...
                        <h2 class="sprint-title">Tablero Sprint 1</h2>
                        <button type="button" class="add-date-btn">
                            ...
                            <span>Añadir fechas</span>
                        </button>
                        <span class="activity-count">(${stories.length} actividades)</span>
                    </div>
                    <div class="sprint-actions">
                        <div class="estimation-badges">
                            <div class="estimation-badge" title="Pendiente: ${backlogPoints} de ${backlogPoints} (puntos de historia)">
                                <span class="badge-number">${backlogPoints}</span>
                            </div>
                            <div class="estimation-badge" title="En curso: ${inProgressPoints} de ${inProgressPoints} (puntos de historia)">
                                <span class="badge-number">${inProgressPoints}</span>
                            </div>
                            <div class="estimation-badge" title="Hecho: ${donePoints} de ${donePoints} (puntos de historia)">
                                <span class="badge-number">${donePoints}</span>
                            </div>
                        </div>
                        <button type="button" class="start-sprint-btn">
                            <span>Iniciar sprint</span>
                        </button>
                        <button type="button" class="sprint-menu-btn" onclick="toggleSprintMenu(event)" title="Más acciones" aria-haspopup="true">
                            <svg fill="none" viewBox="0 0 16 16" role="presentation">
                        ...
                        <div class="sprint-menu-dropdown" id="sprint-menu-dropdown">
                            <button class="sprint-menu-item" onclick="editSprint()">
                                <i class="fas fa-edit"></i>
                                <span>Editar sprint</span>
                            </button>
                            <button class="sprint-menu-item" onclick="deleteSprint()">
                                <i class="fas fa-trash"></i>
    ...
    if (sprintContent) {
        sprintContent.innerHTML = stories.map(story => createBacklogStoryCard(story, members)).join('');
    }
}

function createBacklogStoryCard(story, members) {
    ...
    return `
        <div class="backlog-story-card" data-story-id="${story.id}">
            <div class="story-checkbox">
                ...
                <label for="story-${story.id}"></label>
            </div>
            <div class="story-content">
                <div class="story-header">
                    <span class="story-key">${story.key || 'STORY'}</span>
                    <span class="story-title">${escapeHtml(story.title)}</span>
                    <div class="story-meta">
                        <span class="story-points">${story.storyPoints || 0} pts</span>
                        <span class="story-priority" style="background-color: ${priorityColor}"></span>
                    </div>
                </div>
                ...
                <div class="story-footer">
                    ...
                    <div class="story-actions">
                        ...
                        <button class="btn btn-icon btn-small" onclick="deleteStory('${story.id}')" title="Eliminar">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </div>
...
function toggleSprintSection(button) {
    ...
    if (isExpanded) {
        ...
        button.querySelector('.collapse-icon svg').style.transform = 'rotate(-90deg)';
        button.setAttribute('title', 'Expandir');
    } else {
        sprintContent.style.display = 'block';
        button.setAttribute('aria-expanded', 'true');
        button.querySelector('.collapse-icon svg').style.transform = 'rotate(0deg)';
...
function createStory() {
    ...
    if (sprintContent && createContainer) {
        const storyCardHTML = createBacklogStoryCard(newStory, []);
        if (storyCardHTML) {
